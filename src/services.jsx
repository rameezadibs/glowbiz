...
function ServiceCard({ service, index }) {
  ...
  return <motion.article {...useReveal(index * .09)} whileHover={reduced ? undefined : { y: -6 }} className="service-card">
    ...
    <div className="service-body"><span className="service-icon"><Icon size={31} strokeWidth={1.6} aria-hidden="true" /></span><h3>{service.lines.map(line => <span key={line}>{line}</span>)}</h3><p>{service.description}</p><Link to={`/services/${service.id}`} className="service-link" aria-label={`Learn more about ${service.title}`}><span className="learn-label">Learn More<span className="learn-rule" aria-hidden="true" /></span><span className="service-arrow" aria-hidden="true"><ArrowRight size={21} /></span></Link></div>
  </motion.article>;
}
...
export function ServicesSection({ standalone = false }) {
  ...
  return <section id="services" className="services-section relative isolate overflow-hidden" aria-labelledby="services-heading">
    ...
    <motion.div {...useReveal(.2)} className="services-footer"><span aria-hidden="true"/><Link to={standalone ? '/' : '/services'} className="button button-primary">{standalone ? 'Back to Home' : 'View All Services'}<ArrowRight size={20} aria-hidden="true"/></Link><span aria-hidden="true"/></motion.div></div>
  </section>;
}
